...
- **스테가노 설정**: 1-LSB/2-LSB, 채널(R/G/B) 선택, 픽셀 랜덤화(시드 필요).
- **키 관리**: 키 복사/내보내기/불러오기 지원.
- **검사 도구**: 숨김 데이터 존재 여부와 설정 힌트 확인.
- **공유 체크리스트**: 키와 이미지를 분리 공유, 체크섬(SHA-256) 생성.
- **최근 기록**: 브라우저 로컬에 최근 작업 10개 저장.
